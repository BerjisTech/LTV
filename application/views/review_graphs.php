<script>
    ...
    <?php foreach ($this->db->get('apps')->result_array() as $app) : ?>
        let <?php echo $app['app_code']; ?>_reviews_data = [
            <?php for ($m = 30; $m > 0; $m--) :
                $app_id = $app['app_id'];
    ...
    jQuery(document).ready(function($) {
        drawArea('pc_reviews_chart', pc_reviews_data, reviews_keys, reviews_labels, reviews_colors)
        drawArea('icu_reviews_chart', icu_reviews_data, reviews_keys, reviews_labels, reviews_colors)
        drawArea('pon_reviews_chart', pon_reviews_data, reviews_keys, reviews_labels, reviews_colors)
        drawArea('bdn_reviews_chart', bdn_reviews_data, reviews_keys, reviews_labels, reviews_colors)
        drawArea('wpn_reviews_chart', wpn_reviews_data, reviews_keys, reviews_labels, reviews_colors)
        drawArea('tfx_reviews_chart', tfx_reviews_data, reviews_keys, reviews_labels, reviews_colors)
    })
</script>