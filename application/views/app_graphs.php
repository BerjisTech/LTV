<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		$(".pie-chart").sparkline([
			...
			width: '95',
			height: '95',
			sliceColors: ['#D05421', '#21D1B1', '#C90100', '#C90100', '#C90100', '#E7C00B', '#1E1E1E']
		});
		...
		$(".monthly-sales").sparkline([
			<?php
			foreach ($this->db->select('*, date_format(from_unixtime(install_date), "%Y %m %d") as year, date_format(from_unixtime(install_date), "%m") as month')->where('install_date >=', strtotime('05-05-20121'))->group_by('month')->order_by('year', 'asc')->get('installs')->result_array() as $fetch) {
		...
		function yesterday() {
			var line_chart = Morris.Line({
				...
				xkey: 'y',
				ykeys: ['a', 'b', 'c', 'd', 'e', 'f', 'g'],
				labels: ['Revenue', 'Installs', 'Uninstalls', 'User Growth', 'Ad Spend', '+ Reviews', '- Reviews'],
				lineColors: ['#D05421', '#21D1B1', '#C90100', '#C90100', '#C90100', '#E7C00B', '#1E1E1E'],
				resize: true,
				smooth: true,
			...
			var donut_chart = Morris.Donut({
				...
				colors: ['#D05421', '#21D1B1', '#C90100', '#C90100', '#C90100', '#E7C00B', '#1E1E1E']
			});
		...
		function all_time() {
			...
			<?php $today = $collection_date = date('dmY', time()); ?>
			// Donut Chart
			var donut_chart_demo = $("#donut-chart-demo");
			donut_chart_demo.parent().show();
			var donut_chart = Morris.Donut({
				...
				data: [{
						label: "Product Customizer",
						value: <?php
								$shown = $this->db->where('app_id', 1)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "In Cart Upsell",
						value: <?php
								$shown = $this->db->where('app_id', 2)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "Pre Order Now",
						value: <?php
								$shown = $this->db->where('app_id', 3)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "Bulk Discount Now",
						value: <?php
								$shown = $this->db->where('app_id', 4)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "Wholesale Pricing Now",
						value: <?php
								$shown = $this->db->where('app_id', 5)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "TrackifyX",
						value: <?php
								$shown = $this->db->where('app_id', 6)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "Text 2 Give",
						value: <?php
								$shown = $this->db->where('app_id', 7)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
					...
					},
					{
						label: "SentryKit",
						value: <?php
								$shown = $this->db->where('app_id', 8)->order_by('record_id', 'DESC')->limit(1)->get('quaterly')->row()->value;
								if ($shown == '') {
									echo '0';
								} else {
									echo $shown;
